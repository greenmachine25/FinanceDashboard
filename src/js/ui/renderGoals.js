...
import { cardColors, refreshIcons } from "../utils.js";
...
export function renderGoals(container, state, handlers) {
        ...
        <div id="goal-${goal.id}" class="glass-card flex flex-col h-full">
          ...
          <div class="card-header">
            <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1; min-width: 0;">
              ...
              <input type="text" value="${goal.name}" data-goal-id="${goal.id}" class="goal-name-input"
          ...
          <div class="card-body ${isMin ? "hidden" : ""}" style="${isMin ? "display: none;" : ""}">
            <div class="cards-grid cards-grid-2">
              <div class="input-group">
                <label class="input-label tooltip-help" title="Total amount of money needed for this goal.">Target Amount ($)</label>
                <div class="currency-input-wrapper">
                  <span class="currency-prefix">$</span>
                  <input type="text" inputmode="decimal" value="${goal.targetAmount || ""}" placeholder="5000"
                    class="input-field goal-prop-input" data-goal-id="${goal.id}" data-prop="targetAmount">
                </div>
              </div>

              <div class="input-group">
                <label class="input-label tooltip-help" title="How much currently saved towards this goal.">Currently Saved ($)</label>
                <div class="currency-input-wrapper">
                  <span class="currency-prefix">$</span>
                  <input type="text" inputmode="decimal" value="${goal.savedAmount || ""}" placeholder="1000"
            ...
            <div class="input-group">
              <label class="input-label tooltip-help text-brand-fuchsia" style="color: var(--brand-fuchsia);" title="How much you put towards this goal every month.">
                Monthly Contribution ($)
              </label>
              <div class="currency-input-wrapper">
                <span class="currency-prefix" style="color: var(--brand-fuchsia);">$</span>
                <input type="text" inputmode="decimal" value="${goal.monthlyContrib || ""}" placeholder="200"
            ...
            <div class="input-group">
              <label class="input-label tooltip-help text-brand-teal" style="color: var(--brand-teal);" title="Deducts this monthly contribution from selected budget's wiggle room.">
                <i data-lucide="link" style="width: 0.8rem; height: 0.8rem;"></i> Deduct from Budget
              </label>
              <select class="select-field goal-link-budget" data-goal-id="${goal.id}">
                <option value="">-- None (Standalone) --</option>
                ${state.dashboards
                  .map((d) => `<option value="${d.id}" ${goal.linkedDashboardId == d.id ? "selected" : ""}>${d.name}</option>`)
                  .join("")}
              </select>
            </div>
          </div>
          ...
          <div class="card-footer ${isMin ? "hidden" : ""}" style="${isMin ? "display: none;" : ""}">
            ...
            <div class="metric-box" style="padding: 0.85rem 1rem;">
              <div style="display: flex; align-items: center; gap: 0.5rem; color: var(--text-muted);">
                <i data-lucide="clock" style="width: 1rem; height: 1rem;"></i>
                <span style="font-size: 0.85rem; font-weight: 600;">Time to Goal:</span>
              </div>
              <div style="text-align: right;">
                <span class="font-mono ${metrics.isCompleted ? "text-brand-emerald" : ""}" style="font-weight: 700; font-size: 1.05rem; ${metrics.isCompleted ? "color: var(--brand-emerald);" : ""}">
                  ${metrics.timeRemainingText}
                </span>
                ${metrics.targetDateString ? `<span style="display: block; font-size: 0.725rem; color: var(--text-muted);">${metrics.targetDateString}</span>` : ""}
              </div>
            </div>
          </div>
        </div>
  ...
  container.querySelectorAll(".goal-link-budget").forEach((sel) => {
    sel.onchange = (e) => onUpdateProp(parseInt(e.target.dataset.goalId), "linkedDashboardId", e.target.value);
  });
  ...
  container.querySelectorAll(".color-swatch").forEach((btn) => {
    btn.onclick = (e) => {
      ...
      onUpdateColor("goal", goalId, color);
    };
  });

  initSortableContainer("goalsContainer", "goal", () => {});
  refreshIcons();
}
